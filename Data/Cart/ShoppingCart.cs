...
namespace LarmoireArt.Data.Cart
{
    public class ShoppingCart
    {
        ...
        public List<ShoppingCartItem> GetShoppingCartItems()
        {
            ...
            return ShoppingCartItems ?? (ShoppingCartItems = _context.ShoppingCartItems.Where(n => n.ShoppingCartId == ShoppingCartId).Include(n => n.Produit).ToList());
        }
        ...
        public double GetShoppingCartTotal()
        {
            var total = _context.ShoppingCartItems.Where(n => n.ShoppingCartId == ShoppingCartId)
                                                  .Select(n => n.Produit.Prix * n.Amount).Sum();
            return total;
        }

        //Ajout des articles dans le panier
        public void AddItemToCart(Produit Produit)
        {
            //Recherche d'un Produit existant dans le panier
            var shoppingCartItem = _context.ShoppingCartItems.FirstOrDefault(n => n.Produit.Id == Produit.Id && n.ShoppingCartId == ShoppingCartId);
            //si le Produit n'existe pas encore dans le panier créer un nouvel article
            if (shoppingCartItem == null)
            {
                shoppingCartItem = new ShoppingCartItem()
                {
                    ShoppingCartId = ShoppingCartId,  //Associe l'article au panier actuel
                    Produit = Produit,  //Associe le Produit passé en paramètre à l'article
        ...
        public void RemoveItemFromCart(Produit Produit)
        {
            //Recherche d'un Produit existant dans le panier
            var shoppingCartItem = _context.ShoppingCartItems.FirstOrDefault(n => n.Produit.Id == Produit.Id && n.ShoppingCartId == ShoppingCartId);
            if (shoppingCartItem != null)
